...
!! - [[SHELLSORT_STRINGS]] sort a CHARACTER array
!!
module fpm_strings
...
subroutine shellsort_strings(lines)
character(len=*)             :: lines(:)
character(len=:),allocatable :: ihold
integer                      :: n, igap, i,j,k, jg
   n=size(lines)
   ihold=repeat(' ',len(lines))
   igap=n
   INFINITE: do
      igap=igap/2
      ...
      enddo INNER
   enddo INFINITE
end subroutine shellsort_strings
...
module fpm_filesystem
    ...
    use fpm_environment, only: separator, get_env, run
...
subroutine copy_todir(inputfile,outputdir)
!>
!!##NAME
!!     copy_todir(3f) - [fpm_filesystem:OS] copy a file to specified directory
!!     (LICENSE:MIT)
!!
...
character(len=*),intent(in) :: outputdir
    if(is_dir(outputdir))then
       select case (get_os_type())
       case (OS_UNKNOWN, OS_LINUX, OS_MACOS, OS_CYGWIN, OS_SOLARIS, OS_FREEBSD, OS_OPENBSD)
           call run('cp -f '//inputfile//' '//outputdir)
       case (OS_WINDOWS)
           call run('copy '//inputfile//' "'//outputdir//'"')
       case default   
           call fpm_stop(1, '*copy_todir*:unknown OS type')
       end select
    else
       call fpm_stop(2, '*copy_todir*: output ['//outputdir//'] target is not a directory')
    endif
end subroutine copy_todir
...
type, extends(fpm_cmd_settings)  :: fpm_activate_settings
...
contains
    subroutine get_command_line_settings(cmd_settings)
        ...
        case('activate')
            ...
            & --shell " " &
            & --clear F &
            & --bindir "'//get_env('FPM_BINDIR',' ')//'"' &
            & , help_activate, version_text)
            allocate( fpm_activate_settings :: cmd_settings )
            cmd_settings=fpm_activate_settings(  &
            & bindir=trim(sget('bindir')), &
            & list=lget('list'), &
            & verbose=lget('verbose') )
        ...
        case('help','manual')
            ...
            do i=2,size(unnamed)
                ...
                case('activate' )
                   help_text=[character(len=widest) :: help_text, help_activate]
                case('version' )
   ...
   '         [options]                                                              ', &
   ' ']
   help_activate= [character(len=80) :: &
   'NAME                                                                            ', &
   ...
   ' the latest executables will be stored along with the addition of FPM_BINDIR    ', &
   ' to the beginning of the shell PATH variable.                                   ', &
   '                                                                                ', &
   'OPTIONS                                                                         ', &
   ' --bindir                                                                       ', &
...
!>>>>> ././src/fpm/cmd/activate.f90
module fpm_cmd_activate
!># Definition of the "activate" subcommand
!>
!> A type of the general command base class [[fpm_cmd_settings]]
!> was created for the "new" subcommand ==> type [[fpm_activate_settings]].
...
!>
!> The "activate" subcommand --list --bindir --verbose ## MAYBE: [ --shell ] --clear --directory
!> Once set, should FPM_BINDIR get reused in other projects or be reset?
!> Should clear let you interactively delete and delete by globbing?
!> should check that bindir is a full path name, probably
!> should search directory only be allowed to be created in a fpm build directory?
!> change prompt so know in an activated environment? make it the package name or basename of project?
!> look at "conda activate" to see if any ideas
!> look at module command to see if any ideas
!> Need MSWindows version
!> use fpm_stop instead of error routine
!> --directory option can appear on command but will cause it to fail. Make it work?
!>
...
use fpm_command_line, only : fpm_activate_settings
use fpm_environment, only : run, OS_LINUX, OS_MACOS, OS_WINDOWS
use fpm_filesystem, only : join_path, exists, basename, dirname, mkdir, is_dir
use fpm_error, only : fpm_stop
...
use fpm_environment,  only : get_os_type, get_env, &
                             OS_UNKNOWN, OS_LINUX, OS_MACOS, OS_WINDOWS, &
                             OS_CYGWIN, OS_SOLARIS, OS_FREEBSD, OS_OPENBSD
use fpm_filesystem, only: canon_path, join_path, list_files
use fpm_strings, only: string_t, shellsort_strings
...
subroutine cmd_activate(settings)
type(fpm_activate_settings), intent(in) :: settings
integer,parameter                       :: tfc = selected_char_kind('DEFAULT')
character(len=:,kind=tfc), allocatable  :: pwd_start
character(len=:,kind=tfc), allocatable  :: bindir
character(len=:,kind=tfc), allocatable  :: shell
type(error_t), allocatable              :: error
type(string_t), allocatable             :: file_names(:)
character(len=:,kind=tfc), allocatable  :: name
character(len=:,kind=tfc), allocatable  :: names(:)
integer                                 :: i
integer                                 :: length

    call get_current_directory(pwd_start, error)
    if(.not.is_dir(pwd_start))then
       call fpm_stop(1,'*cmd_activate* '//error%message)
    endif
    bindir=settings%bindir
    if(bindir.eq.'')then
       !x!bindir=canon_path('build/latest') ! assuming OK even if does not exist yet?
       bindir=join_path(pwd_start,'build/latest') ! assuming OK even if does not exist yet?
       ! maybe check if already exists and is a directory
       call mkdir(bindir)
    endif
    if(settings%list)then
       call list_files(get_env('FPM_BINDIR'), file_names)
       if(allocated(file_names))then
          write(*,*)'activated directory:',get_env('FPM_BINDIR')
          if(allocated(names))deallocate(names)
          allocate(character(len=0) :: names(0) )
          do i=1,size(file_names)
             name=basename(file_names(i)%s)
             names=[character(len=max(len(names),len(name))) :: names,name]
          enddo
          call shellsort_strings(names)
          length=0
          do i=1,size(names)
             ...
             write(*,'(1x,a)',advance='no')names(i)
          enddo
          if(length.ne.0)write(*,*)
       endif
    else
       select case (get_os_type())
        case (OS_WINDOWS)
        ! what about powershell, and whatever else there is?
            call run('set PATH='//bindir//';%PATH%;set FPM_BINDIR='//bindir//' cmd.exe')
        case (OS_LINUX, OS_MACOS, OS_CYGWIN, OS_SOLARIS, OS_FREEBSD, OS_OPENBSD, OS_UNKNOWN)
            shell=get_env('SHELL','/bin/bash')
            select case (basename(shell))
            case('bash');shell=shell//' +h' ! turn off hashing or export PROMPT_COMMAND='hash -r'
            end select
            call run('env PATH='//bindir// &
                     & ':$PATH FPM_BINDIR="'//bindir//'" '// &
                     & shell)
        case default
       end select
    endif
...
end subroutine cmd_activate

end module fpm_cmd_activate
...
use,intrinsic :: iso_fortran_env, only : stdin=>input_unit, stdout=>output_unit, stderr=>error_unit
use fpm_error, only : fpm_stop
use fpm_environment, only: run, get_os_type, OS_WINDOWS, get_env
use fpm_filesystem, only: basename, dirname, join_path, exists, mkdir, copy_todir
...
subroutine build_target(model,target,stat)
    ...
    case (FPM_TARGET_EXECUTABLE)
        ...
        if(get_env('FPM_BINDIR').ne.' ')then
           call copy_todir(target%output_file,get_env('FPM_BINDIR'))
        endif
...
use fpm_cmd_activate, only: cmd_activate
...
type is (fpm_activate_settings)
    call cmd_activate(settings)
