...
contains
    ...
    logical function os_is_unix(os) result(unix_os)
...
recursive subroutine list_files(dir, files, recurse)
    ...
    select case (get_os_type())
        ...
        case (OS_WINDOWS)
            !===================================================>>>>
            write(*,*)'DEBUG:LIST_FILES:',windows_path(dir)
            call execute_command_line('dir /b ' // windows_path(dir) )
            !===================================================<<<<
    ...
    files = read_lines(fh)
    !===================================================>>>>
    write(*,*)'DEBUG:SHOW TEMP_FILE:'
    do i=1,size(files)
        write(*,*)'DEBUG:FILES:',files(i)%s
        write(*,*)'DEBUG:DIR:',dir
        write(*,*)'DEBUG:JOIN:',join_path(dir,files(i)%s)
    end do
    !===================================================<<<<
    ...
    write(*,*)'DEBUG:FILES ON EXIT:'
    do i=1,size(files)
        write(*,*)'DEBUG:',i,files(i)%s
    end do
    write(*,*)'DEBUG:EXIT LIST_FILES:'
...
module fpm_toml
    ...
    use tomlf, only : toml_array, toml_key, toml_stat, get_value, &
...
module fpm_manifest_build
    ...
    use fpm_strings, only : string_t
    use tomlf, only : toml_table
    !x!use fpm_toml, only : toml_table
...
module fpm_manifest_package
    ...
    use tomlf, only : toml_table
    !x!use fpm_toml, only : toml_table
    use fpm_toml, only : toml_array, toml_key, toml_stat, get_value, &
        & len
...
module fpm_dependency
  ...
  use tomlf, only : toml_table
  !x!use fpm_toml, only : toml_table
  use fpm_toml, only : toml_key, toml_error, toml_serializer, &
...
contains
  ...
  subroutine load_from_toml(self, table, error)
    ...
    unix_os = get_os_type() /= OS_WINDOWS
...
contains
               ...
               do i=1,ii
                  ...
                  if(line.eq.'')then
                     ...
                     if(verify(line,'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_ -').eq.0.and.line(1:1).ne.' ')then
                        mono_length=len(esc(line))
                        line='<E><g><bo>'//line//repeat(' ',max(0,80-mono_length))
                     else
                        mono_length=len(esc(line))
                        line='<E><w>'//line//repeat(' ',max(0,80-mono_length))
                     endif
                  endif
   ...
   '       except on WINDOWS_OS platforms where it is "<g><bo>never</bo></g><w>".                                       ', &
   ' ']
   ...
   '  fpm-install                                                           ', &
    ...
    '   The default is "auto" except on WINDOW_OS where it is "never".      ', &
    '                                                                       ', &
    'EXAMPLES                                                               ', &
    '   sample commands:                                                    ', &
    '                                                                       ', &
...
use tomlf, only : toml_table
!x!use fpm_toml, only : toml_table
use fpm_toml, only : toml_serializer, set_value
use fpm_manifest_package, only : package_config_t, new_package
use fpm_error, only : error_t
implicit none
character(len=*),intent(in) :: filename
...
use tomlf, only : toml_table
!x!use fpm_toml, only : toml_table
use fpm_toml, only : toml_parse, toml_serializer
implicit none
